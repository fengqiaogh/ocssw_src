/* ============================================================================== */
/* module l1_spexone.c - functions to read SPEX One L1B for l2gen (l1info really) */
/* Written By:  Don Shea                                                          */
/*                                                                                */
/* ============================================================================== */
...
static int lonId, latId, solzId;
static float latFillValue = BAD_FLT;
static float lonFillValue = BAD_FLT;
static float solzFillValue = BAD_FLT;

/**
 * Open the SPEX One L1B file and perform some one-time tasks (as opposed to tasks that
...
int openl1_spexone(filehandle * file) {
    ...
    if (status != NC_NOERR) {
        fprintf(stderr, "-E- Error reading sbins_along_track.\n");
        exit(EXIT_FAILURE);
    }
    nc_inq_dimlen(ncid_L1C, dimid, &num_scans);

    // num_pixels
    status = nc_inq_dimid(ncid_L1C, "spatial_samples_per_image", &dimid);
    if (status != NC_NOERR) {
    ...
    status = nc_inq_varid(groupid, "image_time", &varid);
    if(status != NC_NOERR) {
...
int readl1_spexone(filehandle *file, int32_t line, l1str *l1rec) {
    ...
    check_err(status, __LINE__, __FILE__);
    status = nc_get_vara_float(geolocationGrp, solzId, start, count, l1rec->solz);
    check_err(status, __LINE__, __FILE__);
