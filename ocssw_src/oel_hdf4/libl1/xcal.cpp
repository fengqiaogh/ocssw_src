...
extern "C" double *get_xcal(l1str *l1rec, int type, int wave) {
    ...
    if ((bandnum = xcal_band(wave)) < 0) {
        ...
        // for MODIS L1B-HDF5 files use netCDF xcal LUTs for now
        if (l1rec->l1file->format == FT_MODISL1BH5) {
            sprintf(file, "%s%s%d%s", l1_input->xcal_file, "_", wave, ".nc");
            ...
            try {
                ...
                NcFile cross_cal_lut(filepath, NcFile::read);

                if (cross_cal_lut.isNull()) {
                    std::cerr << "ERROR: Failed to open NetCDF file: " << filepath << '\n';
                    exit(1);
                }

                // dimensions
                NcDim dim = cross_cal_lut.getDim("ncoef");
                if (dim.isNull()) {
                    std::cerr << "Dimension 'ncoef' not found in file: " << filepath << '\n';
                    exit(1);
                }
                ncoef = dim.getSize();
                if (ncoef > XTNORDER) {
                ...
                dim = cross_cal_lut.getDim("ndets");
                if (dim.isNull()) {
                    std::cerr << "Dimension 'ndets' not found in file: " << filepath << '\n';
                    exit(1);
                }
                ndet = dim.getSize();
                if (ndet > XTNDET) {
                ...
                dim = cross_cal_lut.getDim("nmirror");
                if (dim.isNull()) {
                    std::cerr << "Dimension 'nmirror' not found in file: " << filepath << '\n';
                    exit(1);
                }
                nmside = dim.getSize();
                if (nmside > XTNMSIDE) {
                ...
                dim = cross_cal_lut.getDim("ndates");
                if (dim.isNull()) {
                    std::cerr << "Dimension 'ndates' not found in file: " << filepath << '\n';
                    exit(1);
                }
                ntime = dim.getSize();
                if (ntime > XTNTIME) {
                ...
                // yrtab
                NcVar year_table = cross_cal_lut.getVar("year");
                std::memset(yrtab, 0, sizeof(yrtab));  // Initialize all elements to 0
                year_table.getVar(yrtab);

                // dytab
                NcVar day_table = cross_cal_lut.getVar("day");
                std::memset(dytab, 0, sizeof(dytab));  // Initialize all elements to 0
                day_table.getVar(dytab);

                // rvstab
                NcVar m11Var = cross_cal_lut.getVar("M11");
                std::memset(rvstab, 0, sizeof(rvstab));  // Initialize all elements to 0

                std::vector<size_t> begin(4);
                std::vector<size_t> count(4, 1);  // Read exactly one double every call
                for (ic = 0; ic < ncoef; ++ic) {
                    begin[0] = ic;
                    for (id = 0; id < ndet; ++id) {
                        begin[1] = id;
                        for (im = 0; im < nmside; ++im) {
                            begin[2] = im;
                            for (it = 0; it < ntime; ++it) {
                                begin[3] = it;
                                m11Var.getVar(begin, count, &rvstab[ic][id][im][it]);
                            }
                        }
                    }
                }
                ...
                // m12tab
                NcVar m12Var = cross_cal_lut.getVar("m12");
                std::memset(m12tab, 0, sizeof(m12tab));  // Initialize all elements to 0
                for (ic = 0; ic < ncoef; ++ic) {
                    begin[0] = ic;
                    for (id = 0; id < ndet; ++id) {
                        begin[1] = id;
                        for (im = 0; im < nmside; ++im) {
                            ...
                            for (it = 0; it < ntime; ++it) {
                                begin[3] = it;
                                m12Var.getVar(begin, count, &m12tab[ic][id][im][it]);
                            }
                        }
                    }
                }
                ...
                // m13tab
                NcVar m13Var = cross_cal_lut.getVar("m13");
                std::memset(m13tab, 0, sizeof(m13tab));  // Initialize all elements to 0
                for (ic = 0; ic < ncoef; ++ic) {
                    begin[0] = ic;
                    for (id = 0; id < ndet; ++id) {
                        begin[1] = id;
                        for (im = 0; im < nmside; ++im) {
                            ...
                            for (it = 0; it < ntime; ++it) {
                                begin[3] = it;
                                m13Var.getVar(begin, count, &m13tab[ic][id][im][it]);
                            }
                        }
                    }
                }
                ...
                std::cout << "ERROR: FAILURE TO READ NETCDF xcal file" << '\n' << e.what() << std::endl;
                exit(1);
            }
        ...
        }  // END of if() statement to test HDF4 or NetCDF
...
extern "C" double *get_fpm_xcal(char *fpm_file)
{
    FILE *fp;
    char *line = NULL;
    char *ptr = NULL;
    ...
    if ((line = (char*)calloc(bufsz,sizeof(char))) == NULL)
    {
