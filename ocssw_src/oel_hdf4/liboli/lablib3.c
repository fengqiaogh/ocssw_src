...
#endif
{
    ...
    if (tmp_fname == NULL)
        (void)OdlPrintMessage(message_fname,NULL,0,"Unable to create a temporary file");
        ...
        else
        {
            ...
            if (ExpandIsRecursive(kwd, fname))
            {
                (void)sprintf(error_message, 
                        ...
                        keyword_name, fname);
                (void)OdlPrintMessage(message_fname,NULL,(long)kwd->line_number,error_message);
            }
            else
            {
...
#endif
{
    ...
    if (message_fptr != NULL)
        ...
        if (message_fname != NULL && ! odl_suppress_messages)
        {
            ...
            {
                fptr = stdout;
                (void)OdlPrintMessage(NULL, NULL, 0,
...
#endif
{
	...
	(void)strcpy(ODLErrorMessage, line_out);
...
#endif
{
    ...
    for (obj=object; obj != NULL; obj=(OBJDESC *) OdlTraverseTree(obj, object->level))
    {
        ...
        if ((kwd == NULL) || (kwd->value == NULL))
        {
            ...
            (void)OdlPrintLine(message_fname, m_ptr, msgtext);
        }
        else
        {
            (void)sprintf(format, " Line %-5lu %%%lud %%%lus", 
        ...
        if (obj->class == NULL)
            (void)OdlPrintLine(message_fname, m_ptr, "  --  <no class>\n");
        else {
    ...
    if (! odl_suppress_messages)
    {
        ...
        if (object != NULL)
        {
            ...
            if (object->pre_comment != NULL)
                (void)OdlPrintLine(message_fname, m_ptr, object->pre_comment);
	        ...
	        else
		{
	        ...
	        if (object->line_comment != NULL) {
                    (void)OdlPrintLine(message_fname, m_ptr, " ");
                    (void)OdlPrintLine(message_fname, m_ptr, object->line_comment);
		}
            ...
            if (object->post_comment != NULL)
                (void)OdlPrintLine(message_fname, m_ptr, object->post_comment);
		...
		else
		{
                ...
                if (object->end_comment != NULL) {
                    (void)OdlPrintLine(message_fname, m_ptr, " ");
                    (void)OdlPrintLine(message_fname, m_ptr, object->end_comment);
		}
            ...
            if (object->parent == NULL)
                (void)OdlPrintLine(message_fname, m_ptr, "END\n");
...
void OdlPrintKeywords (object, message_fname, message_fptr)
    ...
    if (object != NULL)
    {
                 ...
                 keyword = keyword->right_sibling)
        {
            if (keyword->pre_comment != NULL)
                (void)OdlPrintLine(message_fname, m_ptr, keyword->pre_comment);
 
            (void)OdlPrintLine(message_fname, m_ptr, blanks);

            sfdu_only = FALSE;
            if (keyword->name == NULL)
                (void)OdlPrintLine(message_fname, m_ptr, "unknown_keyword");
            else
            { 
                (void)OdlPrintLine(message_fname, m_ptr, keyword->name);
            ...
            if ((keyword->value != NULL) && (! sfdu_only)) {
                (void)OdlPrintLine(message_fname, m_ptr, " = ");
                (void)OdlPrintLine(message_fname, m_ptr, keyword->value);
            }

            if (keyword->line_comment != NULL) {
                (void)OdlPrintLine(message_fname, m_ptr, " ");
                (void)OdlPrintLine(message_fname, m_ptr, keyword->line_comment);
            }
...

{
    ...
    if (! suppress_metrics && ! suppress_messages) 
    {
        (void)OdlPrintLine(message_fname, m_ptr, "\n");
        (void)OdlPrintLine(message_fname, m_ptr, "           |-------------------------------|\n");
        (void)OdlPrintLine(message_fname, m_ptr, "           | Parsing Metrics:              |\n");
        (void)OdlPrintLine(message_fname, m_ptr, "           |                               |\n");
...
#endif
{
	...
	if (curr_object->parent == NULL)
	{
		...
		status = OdlPrintMessage(message_fname, message_fptr, line_number, errmsg);
	}
...
short OdlValidValueList (text, message_fname, message_fptr, line_number)
