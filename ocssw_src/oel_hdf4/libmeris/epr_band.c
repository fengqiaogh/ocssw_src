...
EPR_SPtrArray* epr_create_band_ids(EPR_SProductId* product_id) {
    ...
    for (i = 0; i < num_descr; i++) {
        ...
        if (strncmp(product_id->id_string, EPR_ENVISAT_PRODUCT_MERIS, 3) == 0
            ...
            band_id->lines_mirrored = EPR_TRUE;
        } else {
            if (strncmp(product_id->id_string, EPR_ENVISAT_PRODUCT_ASAR, 3) == 0
                ...
                if (strncmp(product_id->id_string, EPR_ENVISAT_PRODUCT_SAR, 3) == 0
                    ...
                    band_id->lines_mirrored = EPR_FALSE;
                }
            }
        }
...
float epr_get_scaling_params(EPR_SProductId* product_id,  const char* str) {
    ...
    ziff = epr_get_field_elem_as_float(field, (epr_uint)(scal_fact.elem_index - 1));
    ...
    */
    ...
    if (strcmp(rec_type, "M") == 0) {
        ...
        if (band_id->bm_expr != NULL) {
            ...
            epr_read_bitmask_raster(product_id,
                                            band_id->bm_expr,
                                            offset_x,
                                            offset_y,
                                            bm_raster);
...
int epr_read_band_measurement_data(EPR_SBandId* band_id,
    ...
    for (iY = offset_y; (epr_uint)iY < offset_y + raster->source_height; iY += raster->source_step_y ) {
    ...
    if (band_id->lines_mirrored) {
        if (band_datatype == e_tid_float) {
            ...
            mirror_uchar_array((epr_uchar*)raster->buffer, raster->raster_width, raster->raster_height);
        } else if (band_datatype == e_tid_ushort || band_datatype == e_tid_short) {
            mirror_ushort_array((epr_ushort*)raster->buffer, raster->raster_width, raster->raster_height);
        } else if (band_datatype == e_tid_uint || band_datatype == e_tid_int) {
            mirror_uint_array((epr_uint*)raster->buffer, raster->raster_width, raster->raster_height);
        } else {
            epr_set_err(e_err_illegal_data_type,
                        "epr_read_band_measurement_data: internal error: unknown data type");
            epr_free_record(record);
...
int epr_read_band_annotation_data(EPR_SBandId* band_id,
    ...
    epr_uint rec_numb = 0;
    epr_uint lines_per_tie_pt, samples_per_tie_pt, scan_line_length;
    int iY, raster_pos, delta_raster_pos;
    ...
    for (iY = offset_y; (epr_uint)iY < offset_y + raster->source_height; iY += raster->source_step_y ) {
        ...
        y_beg = (epr_uint)floor(y_mod);

        if (y_beg < 0) {
            y_beg = 0;
        }
        if ((epr_uint)y_beg > dataset_id->dsd->num_dsr - 2) {
...
void* epr_get_raster_elem_addr(const EPR_SRaster* raster, epr_uint offset) {
    ...
    return ((epr_uchar*) raster->buffer) + epr_get_raster_elem_size(raster) * offset;
}
...
void decode_line_uchar_1_of_1_to_float(void* source_array,
                                       ...
                                       int raster_pos) {
    int x, x1, x2;
    epr_uchar* sa = (epr_uchar*) source_array;
...
void decode_line_ushort_1_of_1_to_float(void* source_array,
                                        ...
                                        int raster_pos) {
    int x, x1, x2;
    epr_ushort* sa = (epr_ushort*) source_array;
    float* buf = (float*) raster_buffer;
...
void decode_line_uchar_1_of_1_to_uchar(void* source_array,
    ...
    int x, x1, x2;
    epr_uchar* sa = (epr_uchar*) source_array;
    epr_uchar* buf = (epr_uchar*) raster_buffer;
...
void decode_line_ushort_1_of_1_to_ushort(void* source_array,
    ...
    int x, x1, x2;
    epr_ushort* sa = (epr_ushort*) source_array;
    epr_ushort* buf = (epr_ushort*) raster_buffer;
...
void decode_line_uchar_3_to_i_to_uint(void* source_array,
    ...
    int x, x1, x2, n;
    epr_uchar* sa = (epr_uchar*) source_array;
    epr_uint* buf = (epr_uint*) raster_buffer;
...
void transform_array_ushort_to_float (void* sourceArray,
                                      ...
                                      epr_uint nel) {
    epr_uint ix;
    epr_ushort* sa = (epr_ushort*) sourceArray;
...
void transform_array_uint_to_float (void* sourceArray,
                                     ...
                                     epr_uint nel) {
    epr_uint ix;
    epr_uint* sa = (epr_uint*) sourceArray;
...
void epr_zero_invalid_pixels(EPR_SRaster* raster, EPR_SRaster* bm_raster) {
    ...
    epr_uchar* bm_pixels;
    epr_uint bm_pos = 0;
    epr_uint bm_len = raster->raster_width * raster->raster_height;

    assert(bm_raster->data_type == e_tid_char
           || bm_raster->data_type == e_tid_uchar);


    bm_pixels = (epr_uchar*) bm_raster->buffer;
