...
void set_georegion_filename(const std::string& filename) {
    georegion_filename = filename;
}

extern "C" void set_georegion_filename(const char* filename) {
    std::string s = filename;
    set_georegion_filename(s);
}

extern "C" void close_georegion_file() {
    georegion_file.close();
}

extern "C" int get_georegion(float lat, float lon) {
    if (georegion_variable.isNull()) {
        if(georegion_filename.empty()) {
            std::cerr << "--Error-- " << __FILE__ << ":" << __LINE__ <<  " - The georegion file needs to be set.";
            exit(EXIT_FAILURE);
        }

        std::cout << "Loading the georegion file: " << georegion_filename << std::endl;
        georegion_file.open(georegion_filename, netCDF::NcFile::read);
