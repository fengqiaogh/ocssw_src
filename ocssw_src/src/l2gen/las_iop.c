...
void run_las(l2str *l2rec, int ip) {
    ...
    // Even though these are static arrays they have to be re-allocated each
    // call because bin files have varying npix over scan lines
    //
    if (firstCall) {

        firstCall = 0;

        alloc_las(l1rec->npix, nbands);
...
void iops_las(l2str *l2rec) {
    ...
    for (ip = 0; ip < l2rec->l1rec->npix; ip++) {
        ...
        for (ib = nwave; ib < l2rec->l1rec->l1file->nbands; ib++) {
