...
!      write(91,*) 'KPLANE=',k_plane
      ...
      write(91,*) 'f1a,f2a,f1b,f2b=',f1a,f2a,f1b,f2b
      GCO2   = GGEOM
...
!C
      ...
      DO 310 I=1,NH2O_MAX
        ...
        if (WTRVPR.gt.0) THEN
            ...
            write(91,*) 'SSH2O_S(I,1), I = ', SSH2O_S(I,1), I, SOLZNI*RADEG
            write(91,*) 'SSH2O_S(I,2), I = ', SSH2O_S(I,2), I, OBSZNI*RADEG
        else
...
!C  window and absorption channels used in 3-channel ratioing.
      ...
      write(91,*) 'IWNDW1=',IWNDW1,iwndw1c,' list[0]=',wavobs(1)
...
!C  Common variables
      DIMENSION DP(25), PM(25), TM(25), VMRM(25), G_VAP(25),G_OTHER(25)
      DIMENSION H(25), T(25), P(25), VMR(25)
      DIMENSION WAVOBS(NOBS_MAX),FWHM(NOBS_MAX)
      COMMON /GETINPUT1/ IH2OVP,ICO2,IO3,IN2O,ICO,ICH4,IO2,INO2
      COMMON /GETINPUT3/ H,T,P,VMR,NB,NL,MODEL,IAER,V,TAER55,VRTO3,SNO2
...
!C Calculate 3-channel ratio values, R094 and R114, from simulated spectra.
      ...
      ! R. Healy 9/2015
      ! Reconstruct Transmittances from the k coefficients (from Amir Ibrahim's Matlab Code)
...
!                write(91,*) J,' TRAN_MED_SAP1=',TRAN_MED_INDEX(J,JA_L2),FJM1,FJ,TRAN_MED_INDEX(J-1,JA_L2), &
!                TRAN_MED_INDEX_SAP1(J,JA_L2)
!                write(91,*) J,' TRAN_MED_SAP1_SOL=',TRAN_MED_SAP1(J,1),FJM1,FJ,TRAN_MED_INDEX_SAP1(J-1,1), &
!                TRAN_MED_INDEX_SAP1(J,1)
           ENDIF
!C---
!C---           print*,j,fjm1,fj
!C---
...
!C
       ...
       IF(ISPLITP.NE.0) THEN
          ...
          END DO
!          DO I = 1,NP_STD
!           write(91,*) I,' TRAN_STD_SA_SOL=',TRAN_STD_SA(I,1)
...
!                 write(91,*) K,' TRAN_IA_SA_SOL=',TRAN_IA_SA(1),TRAN_STD_SA(K,1),FINSTR(K-IA(J)+NCVHF(J),J)
...
!            write(91,*) J,' TRNTBL_SOL_SA=',TRNTBLSA(1),TRNTBLSAP1(1),F1A,F2A,F1B,F2B,TRAN_IA_SA(1), &
!                             TRAN_IAP1_SA(1),TRAN_IA_SAP1(1),TRAN_IAP1_SAP1(1)
...
!        write(91,*) j, ' TRAN_STD_OS_SOL=',TRAN_STD_OS(j,1),TRAN_MED_OS(j-NPSHIF,1),TRAN_STD_O(j),TRAN_MED_O(J)
!       write(91,*) j, ' TRAN_STD_OS_SEN=',TRAN_STD_OS(j,2),TRAN_MED_OS(j-NPSHIF,2),TRAN_STD_O(j),TRAN_MED_O(J)
!       end do
       ENDIF
!C The 2nd stage of smoothing - smooth the medium resolution spectrum (resolution
!C     of 0.2 nm and point spacing of 0.1 nm) with about 25,000 points to match
...
!            write(91,*) 'TRNTBL_SO:',TRNTBL_SO(J,1),TRNTBL_SO(J,2),TRAN_IAS(1), &
!             TRAN_IAS(2),TRAN_IAP1S(1),TRAN_IAP1S(2)
           ENDIF
