...
int main(int argc, char *argv[]) {
    ...
    if (input->north != -999 || input->south != -999 || input->east != -999 || input->west != -999 ||
        ...
        if (input->north != -999 && input->south != -999 && input->east != -999 && input->west != -999 &&
            ...
            result = lonlat2pixline1(input->ifile[0], input->geofile, l1_input->resolution, input->west,
                                     input->south, input->east, input->north, &l1_input->spixl,
                                     &l1_input->epixl, &l1_input->sline, &l1_input->eline);

        } else if (input->north == -999 && input->south != -999 && input->east == -999 &&
                   input->west != -999 && input->xbox != -1 && input->ybox != -1) {
            // south, west, xbox, ybox are set
            result = lonlat2pixline2(input->ifile[0], input->geofile, l1_input->resolution, input->west,
                                     input->south, input->xbox, input->ybox, &l1_input->spixl,
                                     &l1_input->epixl, &l1_input->sline, &l1_input->eline);
    ...
    float geospatialLonMax = gring.getGeospatialLongitudeMax();

    for (int ofileIdx = 0; ofileIdx < num_ofiles; ofileIdx++) {
        idDS dsId;
        dsId.fid = ofile[ofileIdx].sd_id;
        if (ofile[ofileIdx].format == FT_L2NCDF)
            dsId.fftype = DS_NCDF;
        else
            dsId.fftype = DS_HDF;
        dsId.deflate = 0;
        ...
        PTB(SetF32GA(dsId, "geospatial_lon_max", geospatialLonMax));
    }

    /*                                                                  */
    /* Write SeaWifs-specific data if appropriate                       */
    /*                                                                  */
    if (l1file.sensorID == SEAWIFS && (l1file.format == FT_SEAWIFSL1A || l1file.format == FT_L1HDF) &&
        ...
        for (i = 0; i < num_ofiles; i++)
            PTB(sd_setattr(ofile[i].sd_id, "Mission", DFNT_CHAR, strlen((const char *)genBuf) + 1,
                           (VOIDP)genBuf));

        bzero(genBuf, sizeof(genBuf));
        PTB(getHDFattr(l1file.sd_id, "Mission Characteristics", "", (VOIDP)&genBuf));
        for (i = 0; i < num_ofiles; i++)
            PTB(sd_setattr(ofile[i].sd_id, "Mission Characteristics", DFNT_CHAR,
                           strlen((const char *)genBuf) + 1, (VOIDP)genBuf));

        bzero(genBuf, sizeof(genBuf));
        PTB(getHDFattr(l1file.sd_id, "Sensor", "", (VOIDP)&genBuf));
        for (i = 0; i < num_ofiles; i++)
            PTB(sd_setattr(ofile[i].sd_id, "Sensor", DFNT_CHAR, strlen((const char *)genBuf) + 1,
                           (VOIDP)genBuf));

        bzero(genBuf, sizeof(genBuf));
        PTB(getHDFattr(l1file.sd_id, "Sensor Characteristics", "", (VOIDP)&genBuf));
        for (i = 0; i < num_ofiles; i++)
            PTB(sd_setattr(ofile[i].sd_id, "Sensor Characteristics", DFNT_CHAR,
                           strlen((const char *)genBuf) + 1, (VOIDP)genBuf));

        bzero(genBuf, sizeof(genBuf));
        PTB(getHDFattr(l1file.sd_id, "Data Type", "", (VOIDP)&genBuf));
        for (i = 0; i < num_ofiles; i++)
            PTB(sd_setattr(ofile[i].sd_id, "Data Type", DFNT_CHAR, strlen((const char *)genBuf) + 1,
                           (VOIDP)genBuf));
