 ...
 *      from Fortran Code by Bo Cai Gao
 *
...
int get_atrem_cor(int32_t sensorID, l1str *l1rec, int32_t ip, float *rhot, float *tg_tot, float *tg_sol, float *tg_sen) {
    ...
    static paramstr P;
    int i, j, k, l, nb, modnum = input->atrem_model;
...
int init_tpvmr(int model) {
    ...
    int i, nb;
    printf("Initializing ATM for model number = %d\n", model);
    model--;
...
int init_tpvmr_nc(int model) {
    ...
    if (!h) {
        ...
        strcat(filename, "\0");
        printf("Initializing arrays for Atrem atmospheric models\n");
...
int init_atrem(int32_t sensorID, paramstr *P, l1str *l1rec, int32_t nbands) {
    ...
    *model = 6;
    ...
    if (getinput6_.wndow2 < 0.6 || getinput6_.wndow2 > 2.5) {
        fprintf(stderr, "Invalid wavelength position for second atmospheric window " \
...
void kdistgasabs(float *kcdf, float *abscf, float*waveno, float *wavobs, int32_t nphi, int32_t n_layers, int32_t nbands) {
    ...
    float *dwave, *dwn, *g_i, *k_i, *g, *kg, *logk, *kint[7], **k7[7], *kk;
    ...
    int32_t nsamp;
    int k, ks, i, j, n, ndxwuv, ndxwir, ndxtot, iw;
...
void model_adjust() {
    ...
    float v = getinput3_.v, \
            taer55 = getinput3_.taer55, vrto3 = getinput3_.vrto3, sno2 = getinput3_.sno2;
    int32_t nl = getinput3_.nl, model = getinput3_.model, iaer = getinput3_.iaer, nb = getinput3_.nb,\
            nobs = getinput5_.nobs, fullcalc = getinput5_.full_calc;
    float hsurf = getinput5_.hsurf, dlt = getinput5_.dlt, dlt2 = getinput5_.dlt2;
    float xpss = getinput14_.xpss, xppp = getinput14_.xppp;

    static float hp[MODELMAX], tp[MODELMAX], pp[MODELMAX], vmrp[MODELMAX];

    float radius_earth = 6380.0, dhk, dhs, tsurf, vmrs, psurf, amtvrt, damtvrt, hplane;
    ...
    fprintf(stderr, "--RJH: Model_adjust: Column vapor amount in model atmosphere from ground\n");
    fprintf(stderr, "--RJH: Model_adjust: to space = %f cm\n", model_adj1_.clmvap);
    /*
    ...
    fprintf(stderr, "--RJH: Model_adjust: Column vapor amount below plane");
    fprintf(stderr, "--RJH: Model_adjust:  = %f cm\n", model_adj3_.clmvapp);

    //Indices and parameters for the plane layer
    //model_adj3_.k_plane = kk;model_adj4_.k_surf = k_surf; // For final C version
    printf("RJH: MODEL_ADJUST - be sure to change next line to model_adj3_.k_plane = kk;model_adj4_.k_surf = k_surf once final conversion to C is complete\n ");
...
void locate_pos_(float *xx, int32_t *n1, float *x1, int32_t *jj) {
    ...
    //  fprintf(stderr,"locate_pos: n=%d x=%f\n",n,x);
    jl = -1;
    ju = n;
    while ((ju - jl) > 1) {
        jm = (ju + jl) / 2;
    ...
    *jj = j;
    //   fprintf(stderr,"Locate end: j=%d jl=%d ju=%d\n",*jj,jl,ju);
    //   return j;
}
...
void geometry() {
    ...
    float mu, mu0;
    int i, j, k_plane = model_adj3_.k_plane, splitpaths = geometry_l2gen_.splitpaths;
    ...
    printf("RJH:GGEOM =%f OBSZNI = %f  OBSPHI = %f solzni=%f degrees :: MU0=%f, MU = %f\n", \
     ...
     *
     */
    printf("RJH: GEOMETRY - be sure to change line to k_plane NOT k_plane-1 once final conversion to C is complete\n ");
    ...
    printf("RJH: GEOMETRY - be sure to change line to k_plane+1 NOT k_plane once final conversion to C is complete\n ");
...
void apply_gas_trans(int32_t k_surf, int levels, int bands, float* abscf, float* dp, float* vmrm, float *tran_hi) {
    ...
    printf("RJH: INIT_SPECCAL - be sure to change below line to i=k_surf and i - k_surf (NOT i - k_surf + 1) once final conversion to C is complete\n ");
...
void init_spectral_calculations() {
    ...
    static int firstCall = 1;
    int32_t i, j, k, k_surf = model_adj4_.k_surf, nl = getinput3_.nl;
    int32_t iwndw1, iwndw2, iwndw3, iwndw4, iwndw5, iwp94c, iw1p14c, nchnla, nchnlb, nchnlc, nchnld, nb1haf, nb2haf, nb3haf, nb4haf;
    ...
    float vrtno2 = 5.0E+15, gno2, sclgas, sclco2, sclch4;
    float *sumcf;
    float tempf;
    ...
    if (firstCall) {
        ...
        for (i = 0; i < NP_MED; i++) {
            ...
            //            tempf = 10000.*DLT_MED/(init_speccal13_.wavln_med_index[i]*init_speccal13_.wavln_med_index[i]);
            //            init_speccal14_.fwhm_wavno[i] = tempf;
            //            init_speccal14_.ncvhf_wavno[i] = ( FACDLT * tempf / DWAVNO + 1.);
            //            init_spectest_.fwhm_wavno[i] = tempf;
            //            init_spectest_.ncvhf_wavno[i] = ( FACDLT * tempf / DWAVNO + 1.);
            //            init_spectest_.tempf = tempf;
        }
        ...
        init_speccal10_.iend2 = nobs - init_speccal10_.ncvhf2;
         ...
         */
        iwndw1 = findMatch(wavobs, nobs, getinput6_.wndow1);
        iwndw2 = findMatch(wavobs, nobs, getinput6_.wndow2);
        ...
        iwp94c = findMatch(wavobs, nobs, getinput6_.wp94c);
        ...
        iwndw4 = findMatch(wavobs, nobs, getinput6_.wndow3);
        iwndw5 = findMatch(wavobs, nobs, getinput6_.wndow4);
        ...
        iw1p14c = findMatch(wavobs, nobs, getinput6_.w1p14c);
...
void tran_table() {
    ...
    int32_t co2 = getinput1_.co2, o2 = getinput1_.o2, n2o = getinput1_.n2o, co = getinput1_.co, ch4 = getinput1_.ch4, no2 = getinput1_.no2, o3 = getinput1_.o3;
    float *fwhm = getinput4_.fwhm, *wavobs = getinput4_.wavobs, *dp = init_speccal5_.dp, *tm = init_speccal5_.tm, *pm = init_speccal5_.pm, *vmrm = init_speccal5_.vmrm;
    float *finst2 = init_speccal10_.finst2, *vmr = getinput3_.vmr;
    int32_t nobs = getinput5_.nobs, nh2o = init_speccal3_.nh2o, fullcalc = getinput5_.full_calc;
    int32_t nb = getinput3_.nb, nl = getinput3_.nl, splitpaths = geometry_l2gen_.splitpaths, ja = geometry_l2gen_.ja, jb = geometry_l2gen_.jb;
    float totlo3 = geometry2_.totlo3, *rno2cf = no2cf_init1_.rno2cf;
    float dlt2 = getinput5_.dlt2, wavcv2, dwvavr, sumins;
    float sno2 = getinput3_.sno2, vrto3 = getinput3_.vrto3, totno2;
    float go3 = geometry2_.go3, Q = model_adj1_.q;
    ...
    if (firstCall) {
        ...
        if (!(tkcdf = (float *) calloc(n_g * nobs * levels, sizeof (float)))) {
            printf("-E- : Error allocating memory to tkcdf\n");
            exit(FATAL_ERROR);
        }

        if (!(sum_kd = (float *) calloc(nobs, sizeof (float)))) {
     ...
     */
    if (firstCall) printf("RJH: TRAN_CAL - be sure to remove below 2 lines (k_surf--;k_plane--) once final conversion to C is complete\n ");
    ...
    if (firstCall || fullcalc) {
        ...
        if (!tran_hi && !(tran_hi = (float *) malloc(NH2OMAX * NP_HI * sizeof (float *)))) {
    ...
    if (!fullcalc) {
        ...
        if (firstCall) {
            //kdist_gas_abs_(tkcdf,abscf_h2o,&nphi,wavno_hi,wavobs,&nobs) ; //FORTRAN
            kdistgasabs(tkcdf, abscf_h2o, wavno_hi, wavobs, bands, levels, nobs); //C
        }
        // Perform the k-distribution calculation Trapezoidal integral for transmittance over the NBANDS bands

        for (i = 0; i < nh2o; i++) {
            for (j = 0; j < nobs; j++) tran_table1_.tran_kd[i][j] = 1.0;

            for (j = k_surf; j < levels; j++) {

                //tkcdf[n_g][nobs][levels]
                for (n = 0; n < nobs; n++) {
                    sum_kd[n] = 0.0;
                    f1 = exp(-tkcdf[0 * nobs * levels + levels * n + j] * dp[j - k_surf] * vmrm[j - k_surf] * ssh2o[i]);

                    for (k = 0; k < n_g - 1; k++) {
                        f2 = exp(-tkcdf[(k + 1) * nobs * levels + levels * n + j] * dp[j - k_surf] * vmrm[j - k_surf] * ssh2o[i]);
...
void tran_smooth(float *tran_hi) {
    ...
    static int32_t ncvhf_wavno[NP_MED], ia[NBANDS], ia_p1;
    int32_t ncvtot_wavno, ncvtot, nobs = getinput5_.nobs, *ncvhf = init_speccal15_.ncvhf, *index_med = init_speccal13_.index_med;
    ...
    if (firstCall) {
        ...
        for (j = 0; j < NP_MED; j++) {
            if (!(finstr_wavno[j] = (float *) malloc(5000 * sizeof (float)))) {
                printf("-E- : Error allocating memory to tran_std\n");
                exit(FATAL_ERROR);
            }
