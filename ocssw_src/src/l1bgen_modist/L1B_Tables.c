...
!Revision History:
 $Log: L1B_Tables.c,v $
 Revision 1.41  2017-03-02 12:55:07-05  xgeng
 Added uncertainty penalty to PV LWIR band

 Revision 1.38  2016-07-22 13:23:31-04  xgeng
 Applied xtalk correction to Terra LWIR PV bands (27-30)

 Revision 1.37  2011-04-07 14:42:40-04  xgeng
 1. RSB &TEB uncertainty algorithm update; 2. The quadratic RSB RVS changed to 4th order.

 Revision 1.35  2008-12-16 16:37:12-05  xgeng
 Extended rvs ascii lut validation limitation

 Revision 1.33  2008/11/18 19:41:07  xgeng
 merge branch for V6.0.0

 Revision 1.32.2.2  2008/06/05 14:07:43  xgeng
 ...
 Revision 1.32  2005/01/18 19:34:46  ltan
 ...
 Revision 02.40  October 21, 2002
 Remove incorrect "&" in data pointer assignment
 of B26_B5_Frame_Offset.
 Alice Isaacman, SAIC GSO  (Alice.R.Isaacman.1@gsfc.nasa.gov) 
 ...
 Revision 02.36  March 15, 2002      Razor Issue #182
  ...
  for (R = 0; R <= INDEX_500M; R++)
  {
    for (B = 0; B < L1A_BANDS_AT_RES[R]; B++)
    {
      for (D = 0; D < DETECT_PER_BAND_AT_RES[R]; D++, D_490++)
      {
        ...
        /*generate noisy and dead subbframe list*/
...
LUT_Definition_t emiss_luts[] = 
{
  ...
  { WAVELENGTH_LUT_NAME,  "wavelength_table.asc",
    ...
    {"NUM_EMISSIVE_DETECTORS", "MAX_NUM_RSR_vs_LAMBDA", 
     NULL, NULL, NULL},
    NULL, True, "3.0", "15.0", "-999." },

  { NUM_WL_INCREMENT_LUT_NAME,  "nwl_table.asc",
    SDS_LUT, DFNT_INT16, 1,
    {NUM_EMISSIVE_DETECTORS, 0, 0, 0, 0},
    {"NUM_EMISSIVE_DETECTORS", NULL, NULL, NULL, NULL},
    NULL, True, "24", "50", NULL },
  ...
  */
  ...
  { XT_UI_FACTOR_LUT_NAME,  
    "xt_ui_factor_table.asc",
    SDS_LUT, DFNT_FLOAT32, 2,
    {NUM_EMISSIVE_BANDS, DETECTORS_PER_1KM_BAND, 0, 0, 0}, 
    {"NUM_EMISSIVE_BANDS", "DETECTORS_PER_1KM_BAND", NULL, NULL, NULL},
    NULL, False, NULL, NULL, NULL },

  { PVLW_XT_COEFF_LUT_NAME,
    "pvlw_xt_coeff_table.asc",
    SDS_LUT, DFNT_FLOAT64, 5,
    {NUM_PVLW_XT_BANDS, DETECTORS_PER_1KM_BAND, NUM_PVLW_XT_BANDS, DETECTORS_PER_1KM_BAND, NUM_PVLW_XT_COEFF},
    {"NUM_PVLW_XT_RECEIVING_BANDS", "DETECTORS_PER_1KM_BAND", "NUM_PVLW_XT_SENDING_BANDS", "DETECTORS_PER_1KM_BAND", "NUM_PVLW_XT_COEFF"},
    NULL, False, NULL, NULL, NULL },
...
*/
{
  ...
  if (sd_id == FAIL || !name || rank <= 0 || rank > MAX_VAR_DIMS || 
...
*/
{
  ...
  int32 dims[MAX_VAR_DIMS];        /* dimensionality of the SDS */
  int32 data_type;                 /* data type of an attributes or the SDS */
  char buffer[MAX_NC_NAME];        /* buffer to store a name */
  char errmsgbuf[PGS_SMF_MAX_MSGBUF_SIZE];
  intn hdf_return;
...
#define TDLUT_STEPFUNCTION_MAX_TIMES 500
  ...
  int32 full_dims[MAX_VAR_DIMS];   /* leading dimension + intrinsic dims */
