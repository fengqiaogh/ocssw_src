...
!Revision History:
 ...
 Revision 1.16  2017-03-01 14:40:43-05  xgeng
 Added uncertainty penalty to PV LWIR bands

 Revision 1.15  2016-07-22 13:23:30-04  xgeng
 Applied xtalk correction to Terra LWIR PV bands (27-30)

 Revision 1.14  2011-04-29 11:55:04-04  ltan
 Typo error correction.

 Revision 1.13  2011-04-07 14:42:58-04  xgeng
 TEB uncertainty algorithm update

 Revision 1.12  2010-11-15 10:38:09-05  xgeng
 No calibration is performed if both sides of PCLW electronics are on at the same time.

 Revision 1.11  2005/01/18 19:34:42  ltan
 MOD_PR02_TERRA update to V5.0.4


 Revision 02.26, March 26, 2003  Razor Issue #191
...
*/
{
  ...
  float32   dn_ev_pvlw_xt_bands[NUM_PVLW_XT_BANDS][DETECTORS_PER_1KM_BAND][EV_1km_FRAMES]; 
  float32   dn_XT_corr = 0;                 /* talk correction. Must be initialized to 0. */
  float32   dn_PVXT_sending = 0;            /* used to calculate the averaged dn_ev */ 
  int16     B_xt         = 0;               /* band index for xt receiving band */
  int16     B_xt_sending, det_tmp;          /* temporary variables */   
  ...
  /* save the dn_ev for PVLW xtalk correction */
  for (B = 0; B < NUM_PVLW_XT_BANDS;  B++)
  {
    B_emiss = PVLW_XT_BANDS[B];
    D_490   = NUM_REFLECTIVE_DETECTORS - DETECTORS_PER_1KM_BAND + B_emiss*DETECTORS_PER_1KM_BAND;
    D_emiss = B_emiss*DETECTORS_PER_1KM_BAND;   
    if ( B_emiss > BAND26) D_emiss -= 10;  /*remove band 26*/ 

    for (D = 0; D < DETECTORS_PER_1KM_BAND; D++, D_emiss++, D_490++)
    {
      
      /* Initialize the sending signal */
      bad_value = False;

      /* Check if the zero point DN value is valid */
      if ( PP_emiss->DN_sv[D_emiss][S] < 0) bad_value = True;

      /* Check to see that the b1 value is not negative. */
      if (PP_emiss->b1[D_emiss][S] < 0) bad_value = True;

      for (F = 0; F < EV_1km_FRAMES; F++)
      {
        dn_ev_pvlw_xt_bands[B][D][F] = 0;
      }

      if (bad_value) continue;

      for (F = 0; F < EV_1km_FRAMES; F++)
      {
        DN_ev = L1A_Scan->EV_1km_night[D][B_emiss][F];
        if (DN_ev > MISSING_L1A_FLAG && DN_ev < SATURATED_DN) 
        { /*only if the data is valid*/
          dn_ev = DN_ev - PP_emiss->DN_sv[D_emiss][S];
          dn_ev_pvlw_xt_bands[B][D][F] = dn_ev; 
        }
      }
    }
  }
                ...
                D++, D_emiss++, D_490++)
    {
      ...
      */
      ...
      for (F = 0; F < EV_1km_FRAMES; F++)
      {
        ...
        {
          if ( B == BAND31 )  dn_ev_31[D][F] = dn_ev;  /* save sending dn */
          ...
          {           /* pc xtalk correction */
            ...
            {
              dn_XT_corr = -emiss_tables->PC_XT[B_xt][D][0]*0.01
                            *dn_ev_31[D][F_shift]
        ...
        /* check if it is a PVLW band [27,28,29, or 30] */
        B_xt = 0; 
        while ( B != PVLW_XT_BANDS[B_xt] ){
          B_xt++;
          if ( B_xt >= NUM_PVLW_XT_BANDS ) break; 
        }  
        if (B_xt < NUM_PVLW_XT_BANDS ) 
        {
          /* need to apply the PVLW cross talk correction */
          dn_XT_corr = 0; 
          for (B_xt_sending = 0; B_xt_sending < NUM_PVLW_XT_BANDS; B_xt_sending++)  /* each sending band */ 
          { 
            F_shift = F + (PVLW_XT_BANDS_FRAME_REL_POS[B_xt] 
                              - PVLW_XT_BANDS_FRAME_REL_POS[B_xt_sending]); 
            if ( F_shift >= 0 && F_shift < EV_1km_FRAMES ) 
            { 
              for (det_tmp = 0; det_tmp < DETECTORS_PER_1KM_BAND; det_tmp++)  /* each sending detector */
              {      
                  dn_PVXT_sending = dn_ev_pvlw_xt_bands[B_xt_sending][det_tmp][F_shift]; 
                  dn_XT_corr += emiss_tables->PVLW_XT_coeff[B_xt][D][B_xt_sending][det_tmp][0] 
                                +emiss_tables->PVLW_XT_coeff[B_xt][D][B_xt_sending][det_tmp][1]*dn_PVXT_sending
                                +emiss_tables->PVLW_XT_coeff[B_xt][D][B_xt_sending][det_tmp][2]
                                *dn_PVXT_sending*dn_PVXT_sending;
              }
            }
          }  
          dn_ev -= dn_XT_corr; 
        ...
        else
        {
          ...
          *****************************************************************/
          ...
          /* 
           * Uncertainty due to cross talk correction (penalty)
           * There is no impact to those bands with no xtalk correction being applied, 
           * since the term dn_XT_corr should be 0, 
           * or the factor can be set to 0.  
           */
          dtemp = emiss_tables->xt_ui_factor[B_emiss][D]*(dn_XT_corr / dn_ev);
          dL_ev += (dtemp * dtemp);
