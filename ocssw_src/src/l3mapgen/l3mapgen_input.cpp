...
int l3mapgenInitOptions(clo_optionList_t* list, const char* softwareVersion) {
    ...
    // files
    clo_addOption(list, "ifile", CLO_TYPE_IFILE, NULL,
                  "input L3 bin filename"
                  "\n");
    ...
    clo_addOption(list, "deflate", CLO_TYPE_INT, "4",
    ...
    clo_addOption(list, "wavelength_3d", CLO_TYPE_STRING, NULL,
                  "comma separated list of wavelengths for 3D products");
    ...
    clo_addOption(list, "projection", CLO_TYPE_STRING, "platecarree",
                  ...
                  "\n");
    clo_addOption(list, "write_projtext", CLO_TYPE_BOOL, "no",
                  "write projection information to a text file.");
    ...
    clo_addOption(list, "north", CLO_TYPE_FLOAT, "-999", "Northernmost Latitude (default: file north)");
    clo_addOption(list, "south", CLO_TYPE_FLOAT, "-999", "Southernmost Latitude (default: file south)");
    clo_addOption(list, "east", CLO_TYPE_FLOAT, "-999", "Easternmost Longitude (default: file east)");
    clo_addOption(list, "west", CLO_TYPE_FLOAT, "-999", "Westernmost Longitude (default: file west)");
    ...
    clo_addOption(list, "apply_pal", CLO_TYPE_BOOL, "yes",
                  ...
                  "\n        yes: color image"
                  "\n         no: grayscale image");
    clo_addOption(list, "palfile", CLO_TYPE_IFILE, NULL, "palette filename (default from product.xml)");
    ...
    clo_addOption(list, "product_rgb", CLO_TYPE_STRING, "rhos_670,rhos_555,rhos_412",
    ...
    clo_addOption(list, "num_cache", CLO_TYPE_INT, "500", "number of rows to cache in memory.");
    clo_addOption(list, "mask_land", CLO_TYPE_BOOL, "no", "set land pixels to pixel value 254");
    clo_addOption(list, "rgb_land", CLO_TYPE_STRING, "160,82,45",
                  "RGB value to use for land mask; comma separate string");
    clo_addOption(list, "land", CLO_TYPE_IFILE, "$OCDATAROOT/common/landmask_GMT15ARC.nc", "land mask file");
...
/**
 * @brief Gets the sensor ID from the input file.
 * 
 * Opens the input file, reads the necessary attributes to determine the sensor
 * ID and returns it (if the sensor ID cannot be determined, defaults to OCRVC)
 * 
...
int l3mapgenReadOptions(clo_optionList_t* list, int argc, char* argv[]) {
    ...
    snprintf(tmpStr, sizeof(tmpStr), "%s/common/l3mapgen_defaults.par", dataRoot);
    ...
    snprintf(tmpStr, sizeof(tmpStr), "%s/%s/l3mapgen_defaults.par", dataRoot, sensorDir);
    int defaultLoaded = 0;
    ...
    if (subsensorId != -1) {
        snprintf(tmpStr,sizeof(tmpStr), "%s/%s/%s/l3mapgen_defaults.par", dataRoot, sensorDir,
    ...
    if (clo_isOptionSet(option)) {
        int suiteLoaded = 0;
        snprintf(tmpStr,sizeof(tmpStr), "%s/common/l3mapgen_defaults_%s.par", dataRoot, clo_getOptionString(option));
        if (access(tmpStr, R_OK) != -1) {
            if (want_verbose)
                printf("Loading default parameters from %s\n", tmpStr);
            clo_readFile(list, tmpStr);
        ...
        snprintf(tmpStr,sizeof(tmpStr), "%s/%s/l3mapgen_defaults_%s.par", dataRoot, sensorDir, clo_getOptionString(option));
        if (access(tmpStr, R_OK) != -1) {
            if (want_verbose)
                printf("Loading default parameters from %s\n", tmpStr);
            clo_readFile(list, tmpStr);
        ...
        if (subsensorId != -1) {
            snprintf(tmpStr,sizeof(tmpStr), "%s/%s/%s/l3mapgen_defaults_%s.par", dataRoot, sensorDir,
                    subsensorId2SubsensorDir(subsensorId), clo_getOptionString(option));
            if (access(tmpStr, R_OK) != -1) {
                if (want_verbose)
                    printf("Loading default parameters from %s\n", tmpStr);
                clo_readFile(list, tmpStr);
