...
int L1C::add_proc_group_l1c(L1C_input* l1cinput, l1c_filehandle* l1cfile, const char* l1c_filename) {
    ...
    status = nc_put_att_text(grp_ip, NC_GLOBAL, str.c_str(), str2.size(), str2.c_str());
    str = "demfile";
    str2 = "$OCDATAROOT/common/gebco_ocssw_v2025.nc";
    status = nc_put_att_text(grp_ip, NC_GLOBAL, str.c_str(), str2.size(), str2.c_str());
...
int32_t L1C::l1_cloud_correct(L1C_input* l1cinput, l1c_filehandle* l1cfile) {
    ...
    if (l1cinput->cloud_correct == 0) {
        ...
        for (unsigned int i = 0; i < l1cinput->files_l1c.size(); i++) {
            ...
            for (size_t i = 0; i < num_ybin; i++) {
                for (size_t j = 0; j < num_xbin; j++) {
                    ...
                    if (l1cinput->verbose)
                        cout << "l1cinput->cloud_height.." << l1cinput->cloud_height << "latnew.."
                             << lat_new[i][j] << "lon_new.." << lon_new[i][j] << "l1clat.." << l1clat[i][j]
                             << "l1clon.." << l1clon[i][j] << endl;
                }
            }
            ...
            if (l1cinput->verbose)
                cout << "corrected L1C grid" << l1c_gran_new << "     granule #" << gran_str << endl;
        ...
        if (l1cinput->verbose)
            cout << "l1cinput->cloud_correct :" << l1cinput->cloud_correct
                 << "LIB->(L1B paralallax corrected)----cloud_height is constant between pixels......"
                 << endl;
        ...
        for (unsigned int i = 0; i < l1cinput->files.size(); i++) {
            ...
            if (l1cinput->verbose)
                cout << "Opening L1B/L2 granule ..." << l1cinput->files[0]
                     << "....for parallax-cloud corrections......." << endl;
            ...
            if (l1cinput->verbose)
                cout << "number of scans.." << num_scans << "num of pixels.." << num_pixels << endl;
            ...
            if (l1cinput->verbose)
                cout << "computing parallax for each pixel.............." << endl;
                ...
                for (unsigned int j = 0; j < num_pixels; j++) {
                    ...
                    if (senzpix[i][j] * scale_factor >= senz_min &&
                        ...
                        dv = Hsat * cth[i][j] * tan(senzpix[i][j] * scale_factor * OEL_PI / 180) /
                             (Hsat - cth[i][j]);
                        lat_new[i][j] =
                            latpix[i][j] * OEL_PI / 180 - dv * cos(temp * OEL_PI / 180 + OEL_PI) / Re;
                        lon_new[i][j] =
                            lonpix[i][j] * OEL_PI / 180 - dv * sin((temp * OEL_PI / 180 + OEL_PI)) /
                                                              (Re * cos(lat_new[i][j] * OEL_PI / 180));
            ...
            if (l1cinput->verbose)
                cout << "corrected L1B grid" << l12_gran_new << "     granule #" << gran_str << endl;
...
int32_t L1C::swtime_swt2_segment(int swt, L1C_input* l1cinput, l1c_filehandle* l1cfile, int32_t norbs,
    ...
    if (tcross > 0) {
        ...
        if (l1cinput->verbose) {
            cout << "tcross " << tcross << "orb_time_tot[ini] " << orb_time_tot[0] << "gn " << gn << "mot "
                 << l1cfile->mot << "gn2 " << gn2 << endl;
    ...
    if (tcross > 0.0) {
        ...
        if (l1cinput->verbose)
            cout << "number of L1C gridlines along-track..." << l1cfile->num_gridlines << "for swath #.."
                 << swt << "t_start.." << t_start << "t_end..." << t_end << endl;
    }  // end equat crossing
    else {
        if (l1cinput->verbose)
...
int32_t L1C::swtime_swt2(int swt, L1C_input* l1cinput, l1c_filehandle* l1cfile, int32_t norbs, double* tswt,
    ...
    if (tcross > 0.0) {
        ...
        if (tg < 0 && l1cinput->verbose) {
            cout << "NEGATIVE FIRST TIME BEFORE CROSSING" << tg << "in swtime_swt2 for mean vel tmgv calc "
                 << mgv << "mot/2 " << mot / 2 << "tcross " << tcross << endl;
        }
...
int32_t L1C::ect_swt(l1c_filehandle* l1cfile, int ix1, int ix2, double* tswt_tot, double* latswt_tot,
    ...
    if (l1cfile->verbose)
        cout << "computing equatorial crossing time and longitude at crossing for a specific swath..orbit "
...
int sunz_swt(int ix1_swt, int ix2_swt, int16_t* hour_swt, int16_t* day_swt, int16_t* year_swt, double* olat,
    ...
    //   if(l1cfile->verbose)cout << "lat1.." << lat1 << "lon1.." << lon1 << "year1" << year1 << "day1" <<
    //   day1 << "hour1" << hour1<< "sunz1..first index" << sunz1 << "ix1" << ix1_swt << "ix2" << ix2_swt <<
    //   endl;
    ...
    // if(l1cfile->verbose)cout << "lat1.." << lat1 << "lon1.." << lon1 << "year1" << year1 << "day1" << day1
    // << "hour1" << hour1<< "sunz1..second index" << sunz1 << "ix1" << ix1_swt << "ix2" << ix2_swt << endl;

    return 1;
}

int32_t L1C::create_time_swt(int num_gridlines, double tfile_ini_sec, double* tmgvf, double tswt_ini_sec,
                             double tswt_end_sec, string* tswt_ini, string* tswt_ini_file, string* tswt_mid,
                             string* tswt_end) {
...
int32_t L1C::open_l1atol1c3(L1C_input* l1cinput, l1c_filehandle* l1cfile) {
    ...
    for (int fi = 0; fi < nfiles; fi++) {
        ...
        if (l1cinput->verbose)
            cout << "secs elapsed.." << secs << "initial granule #..."
                 << round(secs / (l1cfile->gransize * 60)) << endl;
        unix2ymdhms(tfile_ini, &syear, &smon, &sday, &shour, &smin, &second);
        if (l1cinput->verbose)
            cout << "HKT file start time................."
                 << "year.." << syear << "month..." << smon << "day..." << sday << "hour.." << shour
                 << "min.." << smin << "sec..." << second << endl;
        unix2ymdhms(tfile_end, &syear2, &smon2, &sday2, &shour2, &smin2, &second2);
        if (l1cinput->verbose)
            cout << "HKT file end time................."
        ...
        tfile_end_sec =
            tfile_ini_sec +
            49 * 60 * 3;  // 49 minutes per half-orbit, 3x cause 1.5 orbit, forthy swaths (3 before)
        ...
        if (l1cinput->verbose)
            cout << "tfile_ini_sec.."
                 << "YEAR.." << y_ini << "MONTH.." << mo_ini << "DAY.." << d_ini << "HOUR.." << h_ini
                 << "MIN.." << mi_ini << "SEC.." << sec_ini << endl;
        ...
        if (l1cinput->verbose)
            cout << "number_hkpack_tot.." << number_hkpack_tot << "number_scpack_tot.." << number_scpack_tot
        ...
        if (l1cinput->verbose)
            cout << "#number of ephem elements...." << ix.size() << "for HKT file..." << ptstr
                 << "#orecords..." << number_orecords << endl;
        n_ephem = ix.size();
        ...
        if (l1cinput->verbose)
            cout << "total number or orbital records #" << cc - 1 << endl;
        ...
        if (geogap[0] >= 0 && geogap[1] >= 0 && geogap[1] > geogap[0]) {
            if (l1cinput->verbose)
                cout << "interpolating wrong HKT geolocation!! one or more granules must be compromised due "
                        "to "
                        "bad orbitals!! "
        ...
        for (size_t k = 0; k < number_orecords_tot - 1; k++) {
            if (orb_dir_tot[k] != orb_dir_tot[k + 1]) {
                if (swt == 1) {
                    tlimits_swt[swt - 1][1] = k;  // this time is a 'local' end cause may continue circling
                                                  // the earth after ascending or descending
                    swt++;
        ...
        if (l1cinput->verbose)
            cout << "nswath...................." << nswath << "number_orecords_tot.." << number_orecords_tot
                 << endl;
        ...
        for (size_t sw = 0; sw < 2; sw++) {
            if (nswath == 2) {
                ...
                if (sw == 0) {
                    ...
                    if (l1cinput->verbose)
                        cout << "nswath3....swat#1...ix1" << ix1 << "ix2.." << ix2 << "ix3.." << ix3
                             << "ix4..." << ix4 << endl;
                } else if (sw == 1) {
        ...
        if (nswath == 2) {
            if (ix1 >= 0 && ix5 < 0) {
                ...
                if (l1cinput->verbose)
                    cout << "day_mode at nswath #2 SWATH1." << day_mode << endl;
                ...
                if (day_mode == 1) {
                    ...
                    if (l1cinput->verbose)
                        cout << "nswath==2 --tcross equat crossing in (s)..swath#1..." << tcross1
                             << "loncross1..." << loncross1 << endl;
                    ...
                    if (status == 0) {
                        ...
                        swtime_swt2(1, l1cinput, l1cfile, norbs, tswt, tcross1, mgv1, tmgv1, orb_time_tot,
                                    number_orecords_tot);
                        // ini/end times for swath
                        num_gridlines = l1cfile->num_gridlines;
                        if (l1cinput->verbose)
                            cout << "number of across bins L1C grid...#.." << l1cfile->nbinx
                                 << "l1cfile->n_views..." << l1cfile->n_views << endl;
                        ...
                        if (l1cinput->grantype == 1) {
                            ...
                            if (l1cinput->verbose)
                                cout << "estimated # of granules to be processed..." << numgran
                                     << "gd_per_gran..." << gd_per_gran << "#gridlines.." << num_gridlines
                                     << endl;
                    ...
                    if (l1cinput->verbose)
                        cout << "ERROR swath #1 day_mode = " << day_mode
        ...
        if (nswath == 3) {
            if (ix1 >= 0 && ix5 > 0) {
                ...
                if (day_mode == 1) {  // daylight
                    ...
                    status1 = ect_swt(l1cfile, ix1, ix2, orb_time_tot, orb_lat_tot, orb_lon_tot, orb_vel_tot,
                                      grn_vel_tot, tswt, latswt, lonswt, &tcross1, &loncross1, &mov1, &mgv1);
                    if (l1cinput->verbose)
                        cout << "nswath==3 --tcross equat crossing in (s)..swath#1.(segment #1).." << tcross1
                             << "loncross1..." << loncross1 << "orbit direction.0 is descending."
                    ...
                    if (tcross1 < 0.) {
                        ...
                        status2 =
                            ect_swt(l1cfile, ix3, ix4, orb_time_tot, orb_lat_tot, orb_lon_tot, orb_vel_tot,
                                    grn_vel_tot, tswt, latswt, lonswt, &tcross1, &loncross1, &mov1, &mgv1);
                        if (l1cinput->verbose)
                            cout << "nswath==3 -swath1---tcross equat crossing in (s)..swath#1.(segment #2).."
                                 << tcross1 << "loncross1..." << loncross1
                                 << "orbit direction.0 is descending." << l1cfile->orb_dir << "mov1.." << mov1
                                 << "mgv1.." << mgv1 << endl;
                    }  // end segment2
                    ...
                    if (status1 == 0 || status2 == 0 && day_mode == 1) {
                        ...
                        swtime_swt2_segment(
                            1, l1cinput, l1cfile, norbs, tswt, tcross1, mgv1, tmgv1, orb_time_tot,
                            number_orecords_tot);  // number of gridlines may change here!!!!!!!!! not anymore
                                                   // 4000 and assymetric around the equator so bina and binb
                                                   // not the same!!!
                        num_gridlines = l1cfile->num_gridlines;
                        ...
                        if (l1cinput->verbose)
                            cout << "#segment gridlines.ss counter.." << ss << "equal to num_gridlines..."
                                 << num_gridlines << endl;
            ...
            if (l1cinput->verbose)
                cout << "day_mode at nswath #3 SWATH#2: " << day_mode << endl;
            ...
            if (day_mode == 1) {
                ...
                if (l1cinput->verbose)
                    cout << "nswath==3 -swath2--tcross equat crossing in (s)..swath#2..." << tcross2
                         << "loncross2..." << loncross2 << "orbit direction.0 is descending."
                         << l1cfile->orb_dir << "mov2.." << mov2 << "mgv2.." << mgv2 << endl;
                ...
                if (status == 0 && day_mode == 1) {  // crossing and daytime
                    ...
                    swtime_swt2(2, l1cinput, l1cfile, norbs, tswt2, tcross2, mgv2, tmgv2, orb_time_tot,
                                number_orecords_tot);
                    num_gridlines = l1cfile->num_gridlines;
                ...
                cout << "day_mode = 0 nightime...no L1C grid produced--exit (swath#2 "
                        "---nswath#3).............."
                     << day_mode << endl;
                //          exit(1);
            }
...
int L1C::create_SOCEA2(int swtd, L1C_input* l1cinput, l1c_filehandle* l1cfile, float** lat_gd, float** lon_gd,
    ...
    if (l1cinput->verbose)
        cout << "Creating SOCEA proj for the whole orbit......." << endl;
    ...
    if (l1cinput->verbose)
        cout << "sun_earth_distance -- mid gridline" << dist_es << endl;
    ...
    if (Ngring > 0) {
                ...
                for (int i = 0; i < rw; i++) {
                    ...
                    for (int row = 0; row < l1cfile->num_gridlines - 1; row++) {
                        if (latarr[ix] > lat_gd[row][0] && latarr[ix] <= lat_gd[row + 1][0]) {
                            if (l1cinput->verbose)
                                cout << "mid point west# = " << i + 1
                                     << "found index between #row= " << row + 1 << "and row =" << row + 2
                                     << endl;
                            ...
                            lonarr[ix] = lon360;
                            if (l1cinput->verbose)
                                cout << "lon_gd row+1.." << lon_gd[row + 1][0] << "lon_gd row.."
                                     << lon_gd[row][0] << "lon cring.." << lonarr[ix] << endl;
                            break;
                        }
                    }
                }
                ...
                for (int i = 0; i < re; i++) {
                    ...
                    for (int row = 0; row < l1cfile->num_gridlines - 1; row++) {
                        if (latarr[ix] > lat_gd[row][l1cfile->nbinx - 1] &&
                            ...
                            lonarr[ix] = lon360;
                            if (l1cinput->verbose)
                                cout << "lon_gd row+1.." << lon_gd[row + 1][l1cfile->nbinx - 1]
                                     << "lon_gd  row.." << lon_gd[row][l1cfile->nbinx - 1] << "lon cring.."
                                     << lonarr[ix] << endl;
        ...
        for (int s = 0; s < dp; s++) {
            ...
            if (s == 0) {
                onecoor = "POLYGON((" + onelon + " " + onelat + ",";
                firstcoor = onelon + " " + onelat;
            } else
                onecoor = onelon + " " + onelat + ",";
        ...
        for (int row = 0; row < l1cfile->num_gridlines; row++) {
            for (int col = 0; col < l1cfile->nbinx; col++) {
                if (lat_gd[row][col] < latpmin && lat_gd[row][col] != BAD_FLT)
                    latpmin = lat_gd[row][col];
                if (lat_gd[row][col] > latpmax && lat_gd[row][col] != BAD_FLT)
                    latpmax = lat_gd[row][col];
                if (lon_gd[row][col] < lonpmin && lon_gd[row][col] != BAD_FLT)
                    lonpmin = lon_gd[row][col];
                if (lon_gd[row][col] > lonpmax && lon_gd[row][col] != BAD_FLT)
                    lonpmax = lon_gd[row][col];
            }
        }
    ...
    for (pix = 0; pix < num_pixels; pix++) {
        ...
        for (i = 0; i < num_gridlines; i++) {
            ...
            gnvec[0] = sin(l1cfile->lon_gd[i][nbinx - 1] * OEL_PI / 180) *
                       ...
                       sin(l1cfile->lat_gd[i][nbinx - 1] * OEL_PI / 180) *
                           sin(l1cfile->lon_gd[i][0] * OEL_PI / 180) *
                           cos(l1cfile->lat_gd[i][0] * OEL_PI / 180);
            gnvec[1] = sin(l1cfile->lat_gd[i][nbinx - 1] * OEL_PI / 180) *
                           cos(l1cfile->lon_gd[i][0] * OEL_PI / 180) *
                           cos(l1cfile->lat_gd[i][0] * OEL_PI / 180) -
        ...
        if (dot_firstline <= db && dot_lastline > -db) {
            ...
            for (int j = 0; j < nbinx; j++) {
                gvec[0] = cos(l1cfile->lon_gd[irow][j] * OEL_PI / 180) *
                          cos(l1cfile->lat_gd[irow][j] * OEL_PI / 180);
                gvec[1] = sin(l1cfile->lon_gd[irow][j] * OEL_PI / 180) *
                          cos(l1cfile->lat_gd[irow][j] * OEL_PI / 180);
...
int32_t L1C::openL1Cgrid3(l1c_str* l1cstr, l1c_filehandle* l1cfile, L1C_input* l1cinput) {
    ...
    string tswt_ini_file;
    std::string fname_out, senstr, monstr, daystr, yearstr, prodstr, gdstr, swtstr, extstr, granstr, timestr,
        azstr, missionstr, ofilestr;
    NcFile* nc_l1c;
    ...
    if (l1cinput->projection == 0) {  // socea
        ...
        if (l1cinput->verbose)
            cout << "projection type is SOCEA!!!........for grid file..:" << gridname << endl;
    }
    ...
    if (l1cinput->verbose)
        cout << "Opening L1C grid ." << gridname << "for parallax-cloud corrections......." << endl;
    try {
...
int32_t L1C::write_L1C_granule2(int swtd, l1c_filehandle* l1cfile, L1C_input* l1cinput, double* tmgv,
    ...
    int16_t y_ini, mo_ini, d_ini, h_ini, mi_ini, y_end, mo_end, d_end, h_end, mi_end, y_mid, mo_mid, d_mid,
        h_mid, mi_mid, y_zero, mo_zero, d_zero, h_zero, mi_zero;
    ...
    if (l1cinput->start_time[0] != '\0' && l1cinput->end_time[0] != '\0' && l1cinput->grantype == 0) {
        if (l1cinput->verbose)
            cout << "Processing L1C granules between ........................" << l1cinput->start_time
        ...
        if (l1cinput->verbose)
            cout << "ERROR Processing L1C granules between initial and final time --"
                 << "start_time.." << l1cinput->start_time << "end_time.." << l1cinput->end_time
                 << "grantype..." << l1cinput->grantype << endl;
    }
    ...
    if (tmgv[0] < 0)
        twodays = 1;  // the granule can start later even tough the grid contains gridlines from previous day
    ...
    for (int gran = 0; gran < numgran; gran++) {
        for (int i = 0; i < num_gridlines; i++) {
            ...
            // unix2ymdhms(tg_ini, &y_ini, &mo_ini, &d_ini, &h_ini, &mi_ini, &sec_ini);
            // cout<<"gran# "<<gran+1<<"day ini "<<d_ini<<"h ini "<<h_ini<<"mi ini "<<mi_ini<<"sec ini
            // "<<sec_ini<<"tgini "<<tg_ini<<endl;
            //  unix2ymdhms(tg_end, &y_ini, &mo_ini, &d_ini, &h_ini, &mi_ini, &sec_ini);
            // cout<<"gran# "<<gran+1<<"day ini "<<d_ini<<"h ini "<<h_ini<<"mi ini "<<mi_ini<<"sec ini
            // "<<sec_ini<<"tgini "<<tg_ini<<endl; unix2ymdhms(tgridline, &y_ini, &mo_ini, &d_ini, &h_ini,
            // &mi_ini, &sec_ini); cout<<"gridline# "<<i+1<<"gran# "<<gran+1<<"day ini "<<d_ini<<"h ini
            // "<<h_ini<<"mi ini "<<mi_ini<<"sec ini "<<sec_ini<<"tgini "<<tg_ini<<endl;
        ...
        if (l1cinput->start_time[0] != '\0' || l1cinput->end_time[0] != '\0') {
            if (tg_ini >= tg_end || tg_ini >= tgran_end_sec || tg_end > (tgran_end_sec + gransize * 60)) {
                if (l1cfile->verbose)
                    cout << "ERROR : GRAN # " << c + 1
                         << "tg_ini >= tg_end || tg_ini >= tgran_end_sec || tg_end > (tgran_end_sec + "
                            "gransize * 60"
                         << endl;
                break;
            }
    ...
    std::string timestr, missionstr, fname_out, fname_out_nopath, pathstr, monstr, daystr, yearstr, secstr,
        mistr, hstr, prodstr, gdstr, swtstr, extstr, ofilestr, dirstr1, dirstr2, datetimestr1, datetimestr2,
        datetimestr3, fdatetimestr1, date_created, datezerotime;
    ...
    for (int gran = 0; gran < numgran; gran++) {
        ...
        if (twodays) {
            ...
            if (l1cinput->verbose == 1)
                cout << "twodays flag is ON!! : " << twodays << "tzero offset = " << tfile_ini_offset << endl;
        }
        ...
        if (l1cinput->verbose) {
            ...
            cout << "--------------------------" << endl;
            cout << "gran# " << gran + 1 << "day ini " << d_ini << "h ini " << h_ini << "mi ini " << mi_ini
                 << "sec ini " << sec_ini << "tgini " << tg_ini << endl;
            cout << "gran# " << gran + 1 << "day end " << d_end << "h end " << h_end << "mi end " << mi_end
                 << "sec end " << sec_end << "tend " << tg_end << endl;
        ...
        if (tfile_ini_offset + orb_time_tot[0] > tfile_ini_sec + orb_time_tot[norb_rec - 1])
            orb_time_tot[0] -= 24 * 3600;
        if (gdtime[gran][1] > tfile_ini_sec + orb_time_tot[norb_rec - 1])
            torb_off = gdtime[gran][1] - tfile_ini_sec - orb_time_tot[norb_rec - 1];

        if (torb_off > 2 * 60)
            torb_off = 0;

        if (gdtime[gran][0] >= (tfile_ini_offset + orb_time_tot[0]) &&
            gdtime[gran][1] <= (tfile_ini_sec + orb_time_tot[norb_rec - 1] + torb_off)) {
            if (l1cinput->verbose)
            ...
            if (l1cinput->verbose && gtime > gransize) {
                cout << "WARNING: rounding errors --gtime = " << gtime
            ...
            outf << fname_out_nopath << "," << dateini.substr(0, 19) << "," << datend.substr(0, 19) << ","
                 << gfull << "\n";
            ...
            if (lat_gd[NY1 + 1][nadir_bin_index] > lat_gd[NY1][nadir_bin_index]) {
                dirstr1 = "Ascending";
                l1cfile->orb_dir = 1;
            } else {
                dirstr1 = "Descending";
            ...
            if (lat_gd[NY2][nadir_bin_index] > lat_gd[NY2 - 1][nadir_bin_index])
                dirstr2 = "Ascending";
            else
                dirstr2 = "Descending";
