...
Revision History:
   $Log: Granule.c,v $
   Revision 1.21  2012-06-22 07:50:31-04  xgeng
   corrected an error msg
   ...
   Revision 1.15  2005/01/18 21:57:33  ltan
...
!Revision History:
 (continue at top of the file)
   ...
   Revision 1.0.4  October 16, 2004  Razor Issue #200
   ...
   Revision 1.0.3  September 15, 2003
   Add messages regarding "ProcessingEnvironment".
   Liqin Tan, SAIC GSO   (ltan@saicmodis.com)

   Revision 1.0.2  August 26, 2002
   For MODIS/AQUA Processing only
   Added messages for MCST_LUT_VERSION_LUN and WRITE_NIGHT_HIRES_LUN
   Alice Isaacman, SAIC GSO (Alice.R.Isaacman.1@gsfc.nasa.gov)
...
*/
{
  ...
  read_extract_metadata(L1A_Gran->sd_id,
                        &L1A_Gran->Extract_Pixel_Offset,
                        &L1A_Gran->Extract_Pixel_Count,
                        &L1A_Gran->Extract_Line_Offset,
                        &L1A_Gran->Extract_Line_Count);
...
!Revision History:
 (continue at top of the file)
   ...
   Revision 01.04  October 31, 2003 Razor Issue #195
   Added the code of reading the ProcessingCenter value from the
   Run_Time_Parameters_t structure.
   Liqin Tan, SAIC GSO (ltan@saicmodis.com)

   Revision 01.03  September 15, 2003 Razor Issue #181
   Added ProcessingEnvironment parameter as per SDST request for 
   Collection 4. It is set by calling the appropriate POSIX 
   function from within the module, rather than read from the 
   PCF file.  This is to avoid seeing error messages from the 
   SDP Toolkit for each granule when the field is left blank in 
   the PCF file, which is allowable.
   Liqin Tan, SAIC GSO (ltan@saicmodis.com)
...
*/
{
  ...
  if (strcmp(runtime_params->PGE02_Version, PGE02_VERSION))
  {
    returnStatus = MODIS_F_OUT_OF_RANGE;
    char errmsg[512]; 
    sprintf(errmsg, "The PGE02 version in PCF file (%s) does not match the expected value (%s).",
                    runtime_params->PGE02_Version, PGE02_VERSION); 
    L1BErrorMsg(location, returnStatus,
      errmsg, NULL, 0,
