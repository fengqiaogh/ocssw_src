...
!Revision History:
 $Log: L1B.c,v $
 Revision 1.15  2008-11-18 11:20:03-05  xgeng
 merge the branch for V6.0.1

 Revision 1.11.2.3  2008/06/02 15:37:26  xgeng
 Remove the call of Fill_Dead_Detector_SI

 Revision 1.11  2005/01/18 21:57:35  ltan
 Added new file attributes prefixed with HDFEOS_FractionalOffset
   ...
   Revision 02.18  March 26, 2003 Razor Issue #190 
   ...
   Correction had previously been added to Terra code.
...
*/
{
  ...
  setlinebuf(stdout);
  printf("MODIS Aqua L1B version %s, built %s %s\n",
         PGE02_VERSION, __DATE__, __TIME__);
  ...
  for (S = 0; S < L1A_Gran->num_scans; S++ )
  {
    if (S % 10 == 0) {
      time(&tnow);
      printf("scan: %d out of %d %s",
             S, L1A_Gran->num_scans, asctime(localtime(&tnow)));
    }
