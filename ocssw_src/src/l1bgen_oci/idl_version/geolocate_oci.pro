  pro geolocate_oci, l1afile, lutfile, l1bfile, pcdim, psdim, board_id, distcorr, evtime, scana, scans, solz, qflg, iret, EPHFILE=ephfile

; IDL prototype of OCI geolocation program

  print,'Geolocating file ', l1bfile
...
; Get Moon vectors and check for possible eclipse
  l_moon, iyr, iday, evtime, xmoon, rmn
  sdotm = total(sunr[*,sdim/2]*xmoon[*,sdim/2])
  eclipse = 0
  if (sdotm gt 0.9995) then begin
    eclipse = 1
    print,'Checking for solar eclipse'
  endif
...
; Geolocate pixels
    if (eclipse) then rmoon = xmoon[*,iscn]*rmn[iscn]
    oci_geonav, posi(*,iscn), veli(*,iscn), smat, coef, sunr(*,iscn), pview, pcdim, $
    	deltc, xlt, xln, slz, sla, snz, sna, rng, qfl, eclipse, rs[iscn], rmoon
