...
def generate_scalers(args, x_train=None, x_test=None, column_bagging=False):
	...
	if column_bagging and using_feature(args, 'bagging') and (using_feature(args, 'ratio') or many_features):
		n_extra = 0 if not using_feature(args, 'ratio') else RatioTransformer(wavelengths, args.sensor).get_n_features() # Number of ratio features added
		args.x_scalers = [
	...
	if using_feature(args, 'ratio') or using_feature(args, 'all_ratio'):
		kwargs = {}
		if using_feature(args, 'excl_Rrs'):  kwargs.update({'excl_Rrs'  : True})
		if using_feature(args, 'all_ratio'): kwargs.update({'all_ratio' : True})
		args.x_scalers = [
			serialize(RatioTransformer, [list(wavelengths), args.sensor], kwargs),
			serialize(decomposition.TruncatedSVD, [], {'n_components' : 15}),#0.999, 'whiten': False}),#, 'svd_solver': 'randomized'}),
			# serialize(decomposition.PCA, [], {'n_components' : 15}),#0.999, 'whiten': False}),#, 'svd_solver': 'randomized'}),
			# serialize(decomposition.FastICA, [], {'n_components' : 15}),#, 'svd_solver': 'randomized'}),
		] + args.x_scalers
