from .metrics import slope, sspb, mdsa, rmsle
...
def plot_scatter(y_test, benchmarks, bands, labels, products, sensor, title=None, methods=None, n_col=3, img_outlbl=''):
	...
	if len(labels) > 3 and 'chl' not in products:
		...
		if plot_order is None:
			...
			elif len(products) > 1 and any(k in products for k in ['chl', 'tss', 'cdom']):
				plot_order = {k:v for k,v in {
					'chl'  : ['MDN', 'Gilerson_2band'],#,'Smith_Blend'],
					'tss'  : ['MDN', 'SOLID'],#, 'Novoa'],
					'cdom' : ['MDN', 'Ficek'],#, 'QAA_CDOM'],
				}.items() if k in products}
				plot_label = [True] * len(plot_order)
				plot_bands = True
				n_col = len(plot_order)
	plot_order = {p: ['MDN'] for p in products}
	plot_label = [True] * 4
	...
	n_row      = 1#max(1,int(not plot_bands) + int(0.5 + len(plot_order) / (1 if plot_bands else n_col)) -1)
	# if isinstance(plot_order, dict): n_row = 2#len(plot_order)
	# if plot_bands:
	# 	n_col, n_row = n_row, n_col

	fig, axes = plt.subplots(n_row, n_col, figsize=(fig_size*n_col, fig_size*n_row+1))
	axes      = [ax for axs in np.atleast_1d(axes).T for ax in np.atleast_1d(axs)]
	...
	# x_pre  = 'Measured'
	estimate_label = 'Measured' #'Satellite-derived'
	x_pre  = 'Estimated'
	...
	if not isinstance(plot_order, dict):
		full_ax.set_xlabel(xlabel.replace(' ', '\ '), fontsize=24, labelpad=10)
		full_ax.set_ylabel(ylabel.replace(' ', '\ '), fontsize=24, labelpad=10)
	else:
		full_ax.set_ylabel(fr'$\mathbf{{{x_pre}}}$'.replace(' ', '\ '), fontsize=24, labelpad=15)
	...
	for plt_idx, (label, y_true) in enumerate(zip(labels, y_test.T)):
		...
		for est_idx, est_lbl in enumerate(plot_order[product] if isinstance(plot_order, dict) else plot_order):
			...
			ax    = axes[curr_idx]
			cidx  = int(curr_idx / n_row) #(curr_idx / n_col))# if plot_bands else curr_idx
			color = colors[cidx]

			# first_row = curr_idx < n_col 
			first_row = (curr_idx % n_row) == 0
			# last_row  = curr_idx >= ((n_row-1)*n_col) 
			last_row  = ((curr_idx+1) % n_row) == 0
			...
			if est_lbl == 'MDN':
				...
				est_lbl = est_lbl.replace('QAA_CDOM', 'QAA\ CDOM')
			if product not in ['chl', 'tss', 'cdom', 'pc'] and last_col:
				ax2 = ax.twinx()
				ax2.tick_params(labelcolor='none', top=False, bottom=False, left=False, right=False, pad=0)
				ax2.grid(False)
				ax2.set_yticklabels([])
				ax2.set_ylabel(fr'$\mathbf{{{bands[plt_idx]:.0f}nm}}$', fontsize=22)

			minv = -2# if product == 'cdom' else -1 # int(np.nanmin(y_true_log)) - 1 if product != 'aph' else -4
			maxv = 3#1 if product == 'cdom' else 3 # if product == 'chl' else 1 #int(np.nanmax(y_true_log)) + 1 if product != 'aph' else 1
			...
			# if not first_col:                  ax.set_yticklabels([])
				...
				ax.set_xlabel(ylabel.replace(' ', '\ '), fontsize=fs)
			...
			if valid.sum():
				...
				kde = sns.kdeplot(y_true_log[valid], y_est_log[valid], shade=False, ax=ax, bw='scott', n_levels=4, legend=False, gridsize=100, color='#555')# color=color)
			...
			if valid.sum():
				add_stats_box(ax, y_true[valid], y_est[valid], metrics=[mdsa, sspb, slope], fontsize=18)
			...
			if first_row or not plot_bands or (isinstance(plot_order, dict)):# and plot_order[product][est_idx] != 'MDN'):
				if est_lbl == 'BST':
					...
					# ax.set_title(fr'$\mathbf{{\large{{{est_lbl}}}}}$'+'\n'+r'$\small{\textit{(Cao\ et\ al.\ 2020)}}$', fontsize=18)
					ax.set_title(fr'$\mathbf{{\large{{{est_lbl}}}}}$' + r'$\small{\textit{\ (et\ al.\ 2011)}}$', fontsize=fs, linespacing=0.95)
				
				elif est_lbl == 'Mannino':
					# ax.set_title(fr'$\mathbf{{\large{{{est_lbl}}}}}$'+'\n'+r'$\small{\textit{(Cao\ et\ al.\ 2020)}}$', fontsize=18)
					ax.set_title(fr'$\mathbf{{\large{{{est_lbl}}}}}$' + r'$\small{\textit{\ (et\ al.\ 2008)}}$', fontsize=fs, linespacing=0.95)

				elif est_lbl == 'Novoa':
					# ax.set_title(fr'$\mathbf{{\large{{{est_lbl}}}}}$'+'\n'+r'$\small{\textit{(Cao\ et\ al.\ 2020)}}$', fontsize=18)
					ax.set_title(fr'$\mathbf{{\large{{{est_lbl}}}}}$' + r'$\small{\textit{\ (et\ al.\ 2017)}}$', fontsize=fs, linespacing=0.95)

				elif est_lbl == 'GI2B':
					ax.set_title(fr'$\mathbf{{\large{{Gilerson}}}}$' + r'$\small{\textit{\ (et\ al.\ 2010)}}$', fontsize=fs, linespacing=0.95)

				elif est_lbl == 'MDN': ax.set_title(fr'$\mathbf{{{est_lbl}\ {product_labels[product]}}}$', fontsize=fs)
				else: ax.set_title(fr'$\mathbf{{\large{{{est_lbl}}}}}$', fontsize=fs)
