...
class MDN:
	...
	def build(self):
		...
		# model_layers = [tf.keras.Input(shape=(self.batch, self.n_inputs))] + model_layers
		def debug(x):
			tf.print(f'\nShape: {tf.shape(x)}  Min: {tf.reduce_min(x)}  Max: {tf.reduce_max(x)}')
			tf.print(x)
			return x
		# model_layers = [tf.keras.layers.Lambda(debug)] + model_layers
...
class MixtureLayer(tf.keras.layers.Layer):
	...
	def call(self, inputs):
		...
		sigma = tf.einsum('abij,abjk->abik', scale, tf.transpose(scale, perm=[0,1,3,2]))#, scale)
