...
class extract:
    ...
    def run(self):
        ...
        p = subprocess.run(pixlincmd, capture_output=True, text=True, shell=False)
        line = p.stdout
        if not p.returncode:
            pixlin = line.splitlines()[0][2:].split()

            l1extract = os.path.join(self.dirs['bin'], 'l1aextract_modis')
            extractcmd = [l1extract, self.filename, pixlin[0], pixlin[1], pixlin[2], pixlin[3], self.outfile]
            retcode = subprocess.run(extractcmd, shell=False).returncode
            if retcode:
