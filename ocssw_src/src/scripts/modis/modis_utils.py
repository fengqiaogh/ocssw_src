...
def modis_env(self):
    ...
    if self.proctype == "modisGEO":
        ...
        if not os.path.exists(os.path.join(self.dirs['static'],
            ...
            if not os.path.exists(os.path.join(self.dirs['static'],
                ...
                cmd = [os.path.join(self.dirs['bin3'], 'ephtobin'), planetfile]
                status = subprocess.run(cmd, shell=False).returncode
    ...
    if self.proctype == "modisL1B":
        ...
        if not self.lutversion:
            try:
                from seadasutils.LutUtils import lut_version
        ...
        data_files_list = list([mlp.obpg_data_file.ObpgDataFile(self.filename,
                                                            ftype, sensor,
                                                            stime, etime)])
        l1b_name= mlp.get_output_name_utils.get_output_name(data_files_list, 'modis_L1B', None)
        self.base = os.path.join(self.dirs['run'],
                                 os.path.basename(l1b_name).split('.')[0])

        if self.okm is None:
            if re.search('MODIS', l1b_name):
                self.okm = l1b_name
            elif re.search('L1A_LAC', self.filename):
                self.okm = self.filename.replace("L1A_LAC", "L1B_LAC")
            else:
                self.okm = '.'.join([self.base, 'L1B_LAC'])
        if self.hkm is None:
            if re.search('MODIS', l1b_name):
                self.hkm = l1b_name.replace("MODIS", "MODIS_HKM")
            elif re.search('L1A_LAC', self.filename):
        ...
        if self.qkm is None:
            if re.search('MODIS', l1b_name):
                self.qkm = l1b_name.replace("MODIS", "MODIS_QKM")
            elif re.search('L1A_LAC', self.filename):
        ...
        if self.obc is None:
            if re.search('MODIS', l1b_name):
                self.obc = l1b_name.replace("MODIS", "MODIS_OBC")
            elif re.search('L1A_LAC', self.filename):
