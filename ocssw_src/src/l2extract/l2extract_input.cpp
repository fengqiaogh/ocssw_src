 ...
 * @return  0
 */
extern "C" int l2extractInitOptions(clo_optionList_t* list, const char* softwareVersion) {
...
int getSensorId(const char* fileName) { 
    ...
    std::string instrument, platform, Sensor;

    try {
        NcFile nc_input(dataFile, NcFile::read);
        if (dataFile.empty()) {
 ...
 */
extern "C" int l2extractReadOptions(clo_optionList_t* list, int argc, char* argv[]) {
    char* dataRoot;
